...
try:
    ...
    # python3 singularity (i.e. without "-m")

    if __name__ != '__main__':
        # Imported by something else, re-raise the import error
        raise

    first_path = sys.path[0]
    if os.path.isfile(os.path.join(first_path, '__main__.py')) and \
       first_path.endswith('singularity'):
        sys.path.insert(0, os.path.dirname(sys.path[0]))
        from singularity import __full_version__
        
    else:
        # This is "main" and we cannot "fix" the import path;
        # give a human "friendly" error message

        print("Could not locate singularity directory!?")
        print()
